 ...
 *    - The `onSubmit` function is called with the form data.
 *
 * 2. **`onSubmit` Calls `createAccountFn`**:
 ...
 *
 * 3. **`createAccountFn` Calls `createAccount`**:
 *    - The original `createAccount` function is executed with the form data.
 *    - It performs the logic to create an account in the database.
 *
 * 4. **Handling the Response**:
 *    - If `createAccount` succeeds:
 *      - The response is stored in the `data` state.
 *      - The `loading` state is set to `false`.
 *    - If `createAccount` fails:
 *      - The error is stored in the `error` state.
 *      - The `loading` state is set to `false`.
 *
 * 5. **Updating the UI**:
 *    - If `data` is set (success):
 *      - A success toast is displayed.
 *      - The form is reset, and the drawer is closed.
